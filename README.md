...
- the committed local environment file includes placeholders for `accessToken`, `moderatorAccessToken`, `developerEnrollmentRequestId`, `developerEnrollmentAttachmentId`, `notificationId`, `organizationId`, `organizationSlug`, `titleId`, and `titleSlug`
...
- `Board Third Party Library - Local` includes `accessToken`, `moderatorAccessToken`, `developerEnrollmentRequestId`, `developerEnrollmentAttachmentId`, `notificationId`, `authCallbackCode`, and `authCallbackState` placeholders that should be replaced with real values if you want to exercise the authenticated success-path requests against a live backend.
...
- **Developer enrollment**: `GET|POST /identity/me/developer-enrollment` now works with `POST /identity/me/developer-enrollment/{requestId}/cancel`, `GET /identity/me/developer-enrollment/{requestId}/conversation`, `POST /identity/me/developer-enrollment/{requestId}/messages`, and `GET /identity/me/developer-enrollment/{requestId}/attachments/{attachmentId}` so applicants can cancel an open request or continue the same request through a comment-and-attachment thread.
- **In-app notifications**: `GET /identity/me/notifications` plus `POST /identity/me/notifications/{notificationId}/read` provide the current in-app notification surface for developer-enrollment updates.
- **Moderator review**: `GET /moderation/developer-enrollment-requests` plus `POST /moderation/developer-enrollment-requests/{requestId}/approve|reject|request-more-information`, `GET /moderation/developer-enrollment-requests/{requestId}/conversation`, and `GET /moderation/developer-enrollment-requests/{requestId}/attachments/{attachmentId}` provide the full moderator workflow. Approval is backend-mediated and grants the Keycloak `developer` role only after review succeeds.
...
- **Wave 4 public detail**: public title detail includes `mediaAssets` plus a `currentRelease` summary, but not artifact internals or delivery URLs.
- **Release activation**: release state and active-release selection are explicit developer operations; only published releases can be activated.
- **Artifact scope**: artifact endpoints currently manage APK metadata only. Delivery/download URLs remain intentionally absent from the maintained contract.

...
- **Developer enrollment**: `GET|POST /identity/me/developer-enrollment` now represents a direct self-service flow where authenticated users can activate developer access immediately.
- **Verified developer moderation**: `PUT|DELETE /moderation/developers/{developerSubject}/verified-developer` allow moderators to assign or remove the `verified_developer` role for developer accounts.
